...
the proposal in the gateway UI, authorizes it, and runs it. This document
...
| **authorize** | `POST /plans/{id}/authorize` | ❌ 423 | ✅ |
| **run** | `POST /plans/{id}/execute` | ❌ 423 | ✅ |
| discard | `POST /plans/{id}/abort` | ❌ 423 | ✅ |

Authorize, run and abort require `X-Claim-Token`. The token is minted by
...
1. **The step list is hashed and the human authorizes the hash.** Revising a
...
survive a restart, so an authorization must not either.
...
plan reads. There is no authorize/execute/abort tool, and
...
Take control, then **Authorize these N steps**, then **Run**. Two clicks, not
one: authorize records *what was reviewed*, which is what makes a later audit
meaningful, and it keeps the last action before a pipette moves from being a
single click on a screen nobody read.

If the plan changed between the panel rendering it and the click, authorize
